...
- [[source-openai-codex-cookbook-trilogy]] — OpenAI Cookbook's official Codex trilogy (5/9, 5/11, 5/12): Using Goals · Iterative Repair Loops · Agent Improvement Flywheel; 6-element strong-Goal formalism + Review→Repair→Validate phases + 6-step trace-to-harness-change loop
...
- [[source-openai-long-horizon-tasks-codex]] — OpenAI official blog: long-horizon coherence as the true capability (not /goal command); **Prompt.md / Plan.md / Implement.md** triad for durable memory; "freeze the target so the agent doesn't build something impressive but wrong" (~2026-05-05)
- [[source-openai-codex-use-case-follow-goals]] — OpenAI official Codex use case: 5-step setup process for /goal (name + source + validation + checkpoint + monitor) + 3 ready-to-use templates (migration / prototype / prompt-optim); "tighten the goal rather than adding ad hoc instructions" (~2026-05-09)
- [[source-openai-cookbook-plans-md]] — OpenAI Cookbook (**2025-10-07**): AGENTS.md + PLANS.md (ExecPlan) two-file pattern; "more than seven hours from a single prompt"; **chronologically the 2nd independent reinvention of durable-files** (after Ralph, before Matt CONTEXT.md, before triad)
- [[source-openai-codex-hooks-docs]] — OpenAI Codex Hooks (**GA 2026-05-14**): 6 lifecycle events / exit 2 block / managed via `requirements.toml` —— **functional parity with Claude Code hooks**, invalidates pre-5/14 "Anthropic stack thicker" claims
- [[source-openai-codex-automations-docs]] — OpenAI Codex Automations: thread heartbeat + standalone (Triage) + project-scoped (worktree) + cron + `$skill` invocation + `approval_policy = "never"` —— cross-vendor parity with Anthropic routines + `/loop`
- [[source-openai-codex-cloud-environments-docs]] — OpenAI Codex Cloud / Web + Cloud Environments: chatgpt.com/codex unattended runs, 12h container caching, @codex GitHub mention, separated network for setup vs agent —— functional parity with Anthropic cloud routines
- [[source-openai-codex-subagents-docs]] — OpenAI Codex Subagents: default-enabled, `max_threads=6`, `max_depth=1`, 3 built-in (default/worker/explorer), `spawn_agents_on_csv` CSV batch primitive (experimental)
...
- [[source-agent-harness-real-code-waku]] — Sean (Sean's AI Stories) 20-min real-code walkthrough of open-source local agent **Waku-Agent**: the four pillars (harness/loop · memory · eval · LLM ops); retrieval gate + 3 memory types (semantic/episodic/procedural=skills) + deterministic & LLM-as-judge eval + local tracing (YouTube, 2026-07-14; first 精要-mode ingest)
