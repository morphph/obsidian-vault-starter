...
- [[skillify-meta-skill]] — Garry Tan's meta-skill that creates new skills; operationalizes "if I ask twice, you failed"; the "meta-meta" of meta-meta-prompting
- [[book-mirror]] — Two-column chapter-by-chapter book→life mapping pattern; per-section retrieval against personal context graph; 20+ books done, each gets richer because the brain gets richer
...
- [[source-garry-tan-meta-meta-prompting]] — Garry Tan part 6: skillify as production meta-skill, book-mirror pattern, filing-cabinet-vs-nervous-system framing, Karpathy LLM Wiki attribution, 100K-page production scale (2026-05-09, 1.1M views)
...
- [[source-claude-code-agent-teams-docs]] — Official Anthropic agent teams (experimental): coordinated multi-session with shared task list + inter-agent messaging; `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` (2026-05-14 fetch)
- [[source-claude-code-worktrees-docs]] — Official Anthropic worktrees: `--worktree` flag, `.worktreeinclude`, isolation: worktree for subagents, non-git VCS hooks (2026-05-14 fetch)
- [[source-claude-code-mcp-docs]] — Official Anthropic MCP docs: connect external tools, three install scopes, subagent-scoped MCP, channel extension (2026-05-14 fetch)
- [[source-claude-code-plugins-docs]] — Official Anthropic plugins (discover + create + reference): marketplaces, LSP plugins, integrations, scopes, security (2026-05-14 fetch)
- [[source-claude-code-hooks-docs]] — Official Anthropic hooks (guide + reference): lifecycle events, 3 hook types (command/prompt/agent), async/HTTP/MCP tool hooks (2026-05-14 fetch)
- [[source-claude-code-channels-docs]] — Official Anthropic channels (research preview, v2.1.80+): push events into running session via MCP from Telegram/Discord/iMessage/webhooks (2026-05-14 fetch)
- [[source-claude-code-channels-reference-docs]] — Official Anthropic channels server reference: capability declaration, notification format, permission relay (2026-05-14 fetch)
- [[source-claude-code-scheduled-tasks-docs]] — Official Anthropic `/loop` + session-scoped cron tasks: dynamic intervals, built-in maintenance prompt, 7-day expiry, jitter (2026-05-14 fetch)
- [[source-claude-code-routines-docs]] — Official Anthropic routines (research preview): cloud-hosted unattended Claude Code with schedule + API + GitHub triggers (2026-05-14 fetch)
