...
### 2026 Productization (May 2026 update)

**[[sandcastle]] (Matt Pocock, 4.5K stars)** — TypeScript SDK productizes Ralph as `sandcastle.run()`:
- Replaces bash `while true` loop with typed JS API
- Provider-agnostic: Docker / Podman / Vercel Firecracker
- `createWorktree()` materializes interactive→AFK escalation
- `completionSignal` (default `<promise>COMPLETE</promise>`) lets agent signal loop exit
- Reference `.factory/` pattern inside sandcastle repo (plan + implement + review + merge prompts + daemon)

**HITL/AFK labeling** ([[hitl-vs-afk-classification]]) from Matt's `/to-issues`:
- Per-issue label: AFK (agent-executable) or HITL (needs human)
- The dispatcher prerequisite — only AFK-labeled work enters the loop
...
- 2026 ecosystem: [[sandcastle]], [[hitl-vs-afk-classification]], [[idea-to-afk-agent-flow]]
...
| 2026-05-17 | raw/2026-05-17-repo-mattpocock-sandcastle.md | Added Sandcastle as TS productization of Ralph pattern (Matt official) |
| 2026-05-17 | raw/2026-05-17-repo-mattpocock-skills.md | Added HITL/AFK labeling from Matt's /to-issues skill |
| 2026-05-17 | (cleanup) | Removed prior third-party additions (Amplitude, Tessmann, Gekov, Aditya Puri) per user request — only Matt Pocock's official sources retained |
