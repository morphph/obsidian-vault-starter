...
Generate a FAQ page for "claude code model options".
...
### Context
- Primary keyword: claude code model options
- Secondary keywords: claude code common prompts, best practices claude code software development, claude append-system prompt, claude cli system prompt
- Target intent: informational
- Cluster: claude-code-common-workflows
- Available internal links (from same cluster):
  ...
  - Blog: /zh/blog/claude-code-subagents-examples, /zh/blog/how-to-integrate-a-mcp-server, /zh/blog/claude-code-keyboard-shortcuts
  - Compare: none yet
  - FAQ: none yet
...
## Output Requirements
- Output the FULL markdown file including the --- frontmatter block ---
- Frontmatter must include: title, slug (claude-code-model-options), description (120-160 chars), lang (zh), category
- Include related_glossary, related_blog, related_compare fields pointing to the most relevant links above
- slug must be: claude-code-model-options
- lang must be: zh
- Include in frontmatter: related_topics: [claude-code]
...
- slug 保持与英文版相同: claude-code-model-options
- 页面类型: 常见问题
...
Write a faq page for the keyword "claude code model options".
...
如果你想深入了解 Claude Code 的整体架构，可以参考[七层架构深度解析](/zh/blog/claude-code-seven-programmable-layers)；想看模型与 Skills、Hooks 等扩展层如何协作，参见[扩展栈拆解](/zh/blog/claude-code-extension-stack-skills-hooks-agents-mcp)。
...
选型建议：日常功能开发用默认 Sonnet；需要复杂推理（如跨模块重构、安全审计）升级到 Opus；脚本生成、文档补全等低复杂度任务可用 Haiku 降低成本。更多实战技巧见[我每天都在用的 5 个 Claude Code 技巧](/zh/blog/5-claude-code-skills-i-use-every-single-day)。

## 相关问题

- [Claude Code 的 Skills 系统是什么？](/zh/blog/9-principles-writing-claude-code-skills)
- [Claude Code 如何使用 Hooks 实现自动化？](/zh/blog/claude-code-hooks-mastery)
