...
- Builder tools and workflows (Codex, Remotion, MCP, pipelines)
...
- Wiki page format template: `.Codex/rules/wiki-page-format.md` (auto-loaded when editing wiki/)
- Log entry format: `.Codex/rules/log-format.md` (auto-loaded when editing wiki/log.md)

## Commands

Five slash commands. Each has full instructions in `.Codex/commands/`.
...
|-------------|-------------|
| New convention (applies every session) | This file (AGENTS.md) |
| Rule for specific file types/directories | `.Codex/rules/{name}.md` with `paths:` glob |
| New slash command | `.Codex/commands/{name}.md` + add row to Commands table above |
| New skill | `.Codex/skills/{name}/SKILL.md` + add row to Skills table above |
| Skill/command behavior details | Inside the skill/command file, NOT here |

Principle: **AGENTS.md declares WHAT exists. Skills and commands define HOW they work.**

## Compact Instructions

When compressing context, preserve in priority order:
1. Architecture decisions and the four-layer model (raw → wiki → drafts → AGENTS.md)
