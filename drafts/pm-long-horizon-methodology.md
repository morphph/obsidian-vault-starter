...
based-on:
  ...
  - wiki/source-openai-cookbook-plans-md.md
  - wiki/source-openai-codex-hooks-docs.md
  - wiki/source-openai-codex-automations-docs.md
  - wiki/source-openai-codex-cloud-environments-docs.md
  - wiki/source-openai-codex-subagents-docs.md
  - wiki/source-openai-codex-skills-docs.md
  ...
  - wiki/sandcastle.md
companion-guide: drafts/pm-long-horizon-claude-code-practice-guide.md
...
**这里有一个值得 PM 重视的"原型信号"**：**四个独立来源**、不同上下文、不同时间、跨越 10 个月，**独立发明了同一个 pattern**：

| # | 来源 | 文件命名 | 时间 |
|---|---|---|---|
| 1 | Geoffrey Huntley 的 [[ralph-wiggum|Ralph Wiggum]]（社区经典 autonomous coding loop） | `PRD.md` + `progress.txt` + `AGENTS.md` | 2025-07 |
| 2 | **OpenAI Cookbook PLANS.md**（见 [[source-openai-cookbook-plans-md]]）—— 单文件 ExecPlan + AGENTS.md trigger | `AGENTS.md` + `PLANS.md` (ExecPlan) | **2025-10** |
| 3 | Matt Pocock 的 idea-to-AFK 流程（开源 `mattpocock/skills`，87K stars，见 [[grill-with-docs]]） | `CONTEXT.md` + skills 文件 | 2026-04 |
| 4 | OpenAI 官方 long-horizon coherence 博客（见 [[source-openai-long-horizon-tasks-codex]]） | `Prompt.md` + `Plan.md` + `Implement.md` | 2026-05 |

**当四波完全独立的人在不同问题上独立重新发明同一个 pattern，PM 应该把它当成稳定原型来用**，而不是再问"哪家工具更好"。这是行业级 design pattern，不是某家的 feature。

**两种 decomposition 都有效**：
- **单文件 ExecPlan**（PLANS.md 模式）—— setup 快、living document、决策日志和 spec 在一个文档里。适合**个人 indie 项目 / 探索性任务**。OpenAI 给的 case：单 prompt 跑 7+ 小时。
- **多文件 triad**（Prompt.md FROZEN + Plan.md 演进 + Implement.md 行为手册）—— 强制角色分离、FROZEN 标记可被 hook 强 enforce。适合**团队 / 高 stake / 长生命周期项目**。

下文采用 triad 命名作为讲解骨架，因为 FROZEN 那条性质更容易和 Part 3 的 hook 配合讲清楚；但你 PM 实际用时根据复杂度选 single 还是 triad 都对。
...
**这一阶段在 Matt Pocock 的 idea-to-AFK 5-phase 流程里被显式命名为 Phase 4 (Interactive) → Phase 5 (AFK) 的 escalation**（见 [[idea-to-afk-agent-flow]]）。**这一跳的 friction 决定了 AFK 能不能稳定**：跳早了 spec 还没收敛，AFK 跑出垃圾；跳晚了你在 interactive 浪费时间。Matt 的 5-phase 在本文不展开，但作为方法论 spine 之一，与 OpenAI 6-element strong goal、Anthropic /goal 三件套并列 —— 三家从三个角度独立指出同一件事：**强 spec ≠ 完美 spec，强 spec = 经过 calibration 收敛过的 spec**。
...
生产级 AFK 是一个 **5 层 stack**。下面拆开讲，**每层都同时列出 Anthropic Claude Code 和 OpenAI Codex 的等价原语** —— 因为 2026-05 中下旬（**5/14 Codex Hooks GA + Automations + Cloud Environments + Subagents**），两家在这 5 层上已经基本对等。**5 月初我还在写"Anthropic stack 更厚"，5 月中下旬这个判断就不再适用**。下面用 Anthropic 命名做主线，每层后附 Codex 等价。
...
**Codex 等价**：Codex 的 Goal 模式 + ExecPlan（见 [[source-openai-cookbook-plans-md]]）是同样的 stateful loop。Codex 把"循环引擎"做成 thread-scoped stateful state；Anthropic 做成 `/goal` 一等命令。**形状一致**。
...
**Codex 等价**：**Codex Automations 把这三 tier 统一为一个产品概念家族**（见 [[source-openai-codex-automations-docs]]）：

| 三 tier | Anthropic 命名 | Codex 命名 |
|---|---|---|
| In-session | `/loop` + `/goal` | thread automation（heartbeat 风格，保留对话上下文）|
| 本地后台 | `claude --bg` + agent view | project-scoped automation（可选 worktree 隔离）|
| 云端无人值守 | Cloud routines | standalone automation（结果进 Triage inbox）+ Codex Cloud（[[source-openai-codex-cloud-environments-docs]]，**12 小时容器缓存**）|

Codex Automations 支持自定义 cron 语法 + `$skill-name` 调用 skill；unattended 模式默认 `approval_policy = "never"`（**没有 permission prompt，hook 是唯一守门**，跟 Anthropic 一致）。Codex Cloud 的 `@codex` GitHub mention 是一种更轻量的触发方式（不用配 label），但本质上仍在 GitHub event-driven 范畴。
...
**Codex 走了不同路径，没有 channels 直接等价物**：Codex 2026-05-14 ship 的是 **mobile remote-control**（手机连上 Mac 全控 Codex app），philosophy 是"把整个 Codex 搬到手机"，而不是"事件推到第三方 IM"。结果是：

- **Anthropic Channels**：agent 撞到 spec 边界 → 推 Telegram → 你回一句话 → agent 继续。**适合"等多个 agent 同时跑，谁卡了谁找我"**。
- **Codex mobile remote**：你打开 ChatGPT app → 切到 Codex → 看现在每个 thread 状态 → 直接介入。**适合"我要主动 check 进度 + 必要时操作"**。

**对 PM 的选择**：要 push-based 不打扰式提醒 → Anthropic Channels；要 pull-based 主动 check + 全功能控制 → Codex mobile。这是 5 层 stack 里 **唯一一处两家走了不同 philosophy 的层**。
...
**Codex 等价 —— 这是 5/14 之后最关键的对等**：

OpenAI 在 **2026-05-14** 把 **Codex Hooks** 推到 General Availability（见 [[source-openai-codex-hooks-docs]]）。形状与 Claude Code hooks **functional parity**：
...
| Block 机制 | exit 2 → stderr 喂回 agent | exit 2 → stderr 喂回 agent |
| 配置文件 | `.claude/settings.json` | `.codex/hooks.json` 或 `[hooks]` in `config.toml` |
| 配置层 | user / project / plugin | user / project / plugin / **enterprise managed via `requirements.toml`** |
| 拒绝/重写工具调用 | JSON output | JSON `permissionDecision: deny/allow` + `updatedInput` |

Codex 配置例子（与前面 Claude Code 等价）：
...
**前面写"Anthropic 在 determinism 层独有"是 2026-05-05 时的判断；2026-05-14 之后这个差距消失**。**这一层 PM 选 vendor 不再因为"谁有 hooks"，而是因为你团队下游生态是哪边**。
...
**Codex 等价**：Codex Subagents（见 [[source-openai-codex-subagents-docs]]）默认支持并行，`max_threads=6` 上限，`max_depth=1` 防递归爆炸，3 个 built-in agent（default / worker / explorer）+ TOML 自定义 agent（每个可独立指定 `model` / `sandbox_mode` / `mcp_servers`）。**Anthropic 的 first-class `agent view` TUI 在 Codex 里目前以 `codex cloud` interactive picker + Triage inbox 形态出现，体验略不同但功能等价**。**额外的 Codex-only 原语**：`spawn_agents_on_csv` 把"对 1000 个文件做同样审计"做成 first-class 批处理原语 —— Anthropic 没有直接等价物，要自己写 loop。
...
## 收尾：跨厂商收敛 + agentskills.io 共标准 = PM craft 升级窗口
...
- **Durable memory pattern 至少 4 个独立来源**：Ralph Wiggum（2025-07）/ **OpenAI Cookbook PLANS.md**（2025-10，[[source-openai-cookbook-plans-md]]）/ Matt Pocock 的 CONTEXT.md（2026-04）/ OpenAI 官方 Prompt+Plan+Implement（2026-05）—— **跨越 10 个月、4 波完全独立的人** 收敛到同一个 pattern
- **"tighten the goal, don't patch"**：OpenAI Codex 官方 docs 独立写下、Matt Pocock 的 interactive→AFK escalation 同源
- **Hooks GA 在两家**：Anthropic 早；**Codex 2026-05-14 GA**（[[source-openai-codex-hooks-docs]]）—— 5 月中下旬之后，determinism 层不再是 Anthropic 独有

当一个东西在几周内被多家独立 reinvent，**它就不是 vendor feature 了，是 industry pattern**。

### 跨厂商收敛升级：从软证据到硬证据

到 2026-05 为止，"独立发明同一 pattern"还是 *软证据* —— 行业 convergent evolution。**但 OpenAI Codex Skills 显式 build on `agentskills.io` 开放标准（见 [[source-openai-codex-skills-docs]]），这是 *标准化* 的硬证据**：

- **Anthropic Skills 和 Codex Skills 共用同一文件格式标准**（SKILL.md frontmatter / progressive disclosure / 8K 字符初始 prompt budget）
- 移植一个 Matt Pocock-style Claude Code skill 到 Codex skill：rename 目录 `.claude/skills` → `.agents/skills`、invocation syntax `/skill` → `$skill`、可选加 `agents/openai.yaml`。**典型 < 1 小时**
- **战略含义**：Matt 的 `mattpocock/skills`（87K stars）从 *Claude Code-only 资产* 升级为 *跨 vendor 可分发资产*。短期内大概率出现 cross-vendor skill marketplace

**对 PM 的判断**：以前 "选 vendor" 意味着"选生态锁定"；agentskills.io 共标准之后，skill 是 portable 的，**锁定变浅**。重要决定变成：

| 你的下游生态 | 倾向选 |
|---|---|
| 用 Matt Pocock tooling（grill-with-docs / Sandcastle）| Anthropic Claude Code（Matt 的代码现在跑这里）|
| 团队已 ChatGPT Enterprise / IT 已批 OpenAI | OpenAI Codex |
| 需要 Telegram / Discord push-based 提醒 | Anthropic Channels |
| 需要 mobile pull-based 完整远程操作 | Codex mobile remote |
| 需要 GitHub `@codex` 一键触发 | Codex Cloud |
| 需要 HTTP POST webhook 触发 | Anthropic Cloud Routines |
| 需要 CSV 批量审计原语 | Codex `spawn_agents_on_csv` |
| 需要本地 agent view TUI 看 fleet 状态 | Anthropic |

**5 层 stack 概念 vendor-agnostic；具体落地选 vendor 是次要决策**。
...
配套的实操指南（[[pm-long-horizon-claude-code-practice-guide|drafts/pm-long-horizon-claude-code-practice-guide.md]]）拿 blog2video pipeline 里的 **chapter splitter** 作为练习 case，**用 Claude Code 跑一遍**：把上面 5 个动作 + 5 层 stack 完整走一遍：
...
> 📌 Codex 实操篇（future work）
>
> 当前 practice guide 是 Claude Code 版，因为 Matt Pocock 的 `grill-with-docs` / Sandcastle 工具链当前只跑在 Claude Code 上。**Codex 5 层 stack 在 2026-05 已经 ready**（hooks GA / automations / cloud / subagents / skills），但 Matt 的 skill 库尚未移植到 `.agents/skills` 格式。
>
> 等三件事之一发生，写 Codex 版 practice guide：
> 1. Matt 把 `mattpocock/skills` 适配出 agentskills.io 跨 vendor 版本（社区移植大概率会先发生）
> 2. 你自己用 Codex Cloud + PLANS.md 跑通一次 blog2video（个人验证）
> 3. 出现 cross-vendor skill marketplace（agentskills.io 标准化的下一步）

---

## 来源

**Anthropic Claude Code 官方 docs**：
...
**OpenAI Codex 官方 docs（2026-05 之后 5 层 stack 已对等）**：
...
- [[source-openai-codex-use-case-follow-goals]] — OpenAI Codex 官方 use-case "Follow a Goal"
- [[source-openai-cookbook-plans-md]] — **OpenAI Cookbook PLANS.md (2025-10)** —— 单文件 ExecPlan / AGENTS.md trigger / "7+ hours from single prompt"
- [[source-openai-codex-hooks-docs]] — **Codex Hooks (GA 2026-05-14)** —— 与 Claude Code hooks functional parity
- [[source-openai-codex-automations-docs]] — Codex Automations —— thread / standalone / project-scoped 三类 + cron + `$skill`
- [[source-openai-codex-cloud-environments-docs]] — Codex Cloud / Web + Cloud Environments —— 12h container cache + `@codex` GitHub trigger
- [[source-openai-codex-subagents-docs]] — Codex Subagents —— `max_threads=6` + `spawn_agents_on_csv` 批处理原语
- [[source-openai-codex-skills-docs]] — Codex Skills —— build on `agentskills.io` 开放标准（与 Anthropic Skills 共标准）
